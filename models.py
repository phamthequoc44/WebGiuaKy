...
class Task(db.Model):
    ...
    # projectId = db.Column (db.Integer,ForeignKey('projects.projectId'))
    # statusId = db.Column (db.Integer,ForeignKey('status.statusId'))    
...
class Status(db.Model):
    statusId = db.Column(db.Integer, Sequence('status_id_seq'), primary_key=True)
    description = db.Column (db.String(255),nullable=False)

class Projects(db.Model):
    projectId = db.Column (db.Integer,Sequence('project_id_seq'),nullable=False,primary_key=True)
